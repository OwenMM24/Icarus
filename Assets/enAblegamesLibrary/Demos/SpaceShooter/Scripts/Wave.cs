...
namespace SpaceShooterDemo
{
    public class Wave : MonoBehaviour
    {
        ...
        IEnumerator CreateEnemyWave() //depending on chosen parameters generating enemies and defining their parameters
        {
            for (int i = 0; i < count; i++)
            {
                ...
                if (pathPoints[0].position.x > pathPoints[1].position.x)
                {
                    enemyComponent.movingRight = false;
                }
                enemyComponent.shotChance = DDAManager.Instance.DifficultyValues["EnemyAggression"] * 3;
                print("UPDATE DDA TEST: DDA FOR ENEMYAGGRESSION IS " + DDAManager.Instance.DifficultyValues["EnemyAggression"]);

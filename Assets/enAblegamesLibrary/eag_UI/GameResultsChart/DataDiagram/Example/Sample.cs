...
public class Sample : MonoBehaviour {
    ...
    public void OnButton(Vector2 dataPoint) {

        if (null == m_DataDiagram)
            return;

        foreach (GameObject x in lineList) {
            //Vector2 dataPoint = new Vector2(1, Random.value * 4f);
            m_DataDiagram.InputPoint(x, dataPoint);
            print(dataPoint);
        }
    }
